...
Scenario Outline: Add new job
	...
	When the admin saves new job with valid 'job_titile', 'job_description' and 'note'
	Then a job with 'job_titile' and with 'job_description' exist in table

Examples:
 |job_titile       |job_description                                   |note                                                           |
...
Scenario Outline: Delete job
	Given a admin on the admin viewJobTitleList page
	And a job with 'job_titile' exist in table
	When the admin delete a job with 'job_title' through Delete Selected button 
	Then row with 'job_titile' don`t exist in table
